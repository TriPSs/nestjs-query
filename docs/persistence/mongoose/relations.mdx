...
<Note>There are multiple ways to set of references in `mongoose` there are intended as starting point.</Note>

<Warning>Filtering on references is not supported by mongoose.</Warning>

...
description: 'The @ptc-org/nestjs-query-rest package creates documented REST CRUD controllers backed by a QueryService. It provides DTO decorators, generated controllers, filtering, offset paging, hooks, authorization, and CSV export.'
---
...
Register the persistence module and describe the endpoint in `NestjsQueryRestModule.forFeature`. `basePath` is optional; without it, the path is derived by pluralizing the DTO class name (`TodoItemDTO` becomes `/todo-item-dtos`).

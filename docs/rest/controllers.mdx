...
CSV exports serialize `Date` values as UTC ISO 8601 strings (for example, `2026-09-20T12:30:00.000Z`). Use `@ExportTransform()` from `@ptc-org/nestjs-query-rest` to format a field only for CSV export, for example `@ExportTransform(({ value }: { value: Date }) => value.toISOString().slice(0, 10))`. It accepts the same callback as `@Transform()` but does not run during ordinary DTO conversion or API serialization. Keep `@Expose()` on exported properties.

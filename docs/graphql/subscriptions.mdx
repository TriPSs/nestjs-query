...
<Warning>
  The plain Mercurius driver cannot serve as a federation subgraph. Use `MercuriusFederationDriver` (also exported by
  `@nestjs/mercurius`) instead: it builds the subgraph schema through `@apollo/subgraph`, supports `@ResolveReference`, and
  patches subscriptions so they keep working. Note that the Mercurius gateway only composes federation v1 SDL, so subgraphs should
  not be configured with a federation v2 `autoSchemaFile`.
</Warning>

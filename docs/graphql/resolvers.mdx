...
- `export` - Configure CSV export. See [CSV Export](/graphql/export).
  ...
  - `limit` - Set the maximum number of exported records. Defaults to `1000`. Only the first `limit` matching records are exported; additional records are omitted without an error or truncation indicator.

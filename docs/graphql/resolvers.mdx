...
<Warning>
  When using the `@Resolver` decorator from `@nestjs/graphql` you must use the following otherwise relations will not work.
...
- `exportTodoItems` - Export filtered `TodoItems` as CSV, only when `export.enabled` is `true`. The query name follows `export<PluralName>`.
...
- `export` - Configure CSV export. See [CSV Export](/graphql/export).

  - `enabled` - Export is disabled by default. This must be explicitly set to `true` to add the export query to the GraphQL schema.
  - `limit` - Set the maximum number of exported records. Defaults to `1000`. Clients can request fewer records with the query’s `paging.limit` argument and select a page with `paging.offset`. The requested limit is capped at this maximum; additional records are omitted without an error or truncation indicator.
  - `ExportDTOClass` - Use a separate `@ObjectType()` DTO whose GraphQL fields define the allowed export columns, including inherited fields. Supports `class-transformer` transformations without requiring `@Expose()`.
  - Select CSV columns and headers using the query's required `fields` argument.

#### `ResolverOptions`[​](#resolveroptions 'Direct link to resolveroptions')

The `create`, `read`, `update`, `delete`, and `export` options above all accept the following options.

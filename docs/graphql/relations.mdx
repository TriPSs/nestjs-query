---
title: 'Relations'
description: 'When using the  you can specify relations that should be exposed for the DTO using the following decorators.'
---
...
<Note>
  When loading relations a [dataloader](https://github.com/graphql/dataloader) that is scoped to the request will be used. This
  prevents the n+1 problem.
</Note>

<Warning>
  When upgrading to NestJS 12 (`@nestjs/graphql@14`), an explicitly declared field takes precedence over a relation with the
  same name on the same DTO. In `@nestjs/graphql@13` the relation used to win instead. If a DTO declares both — for example a
  class-level `@Relation('lender', () => LenderDto)` and a `@FilterableField(() => GraphQLObjectID, { nullable: true }) lender: string`
  — the generated schema will silently change after the upgrade. Remove the duplicate declaration and keep only the one you
  actually want exposed. See [issue #532](https://github.com/TriPSs/nestjs-query/issues/532) for details.
</Warning>

In the following examples we are going to use the following entities.

<Tabs>
  <Tab title="TodoItemEntity">
    ```ts todo-item/todo-item.entity.ts
  ...
  <Tab title="SubTaskEntity">
    ```ts sub-task/sub-task.entity.ts
    import {
...
```ts sub-task/sub-task.module.ts
...
<Note>
  If `remove.enabled` was set to `true` a `removeTodoItemFromSubTask` mutation would also be exposed with the same arguments as
  `setTodoItemOnSubTask`.
</Note>

## @FilterableRelation[​](#filterablerelation 'Direct link to @FilterableRelation')
...
<Warning>
  The `@FilterableRelation` decorator will **only** work with relations defined by the orm used (e.g. `typeorm`, `sequelize`). If
  your relations are federated or you are using `mongoose` you cannot use the `@FilterableRelation` decorator.
</Warning>
...
## @UnPagedRelation[​](#unpagedrelation 'Direct link to @UnPagedRelation')
...
- `todoItem.subTasks` - A `query` endpoint to retrieve a `TodoItems` `SubTasks`.
  ...
  - The `subTasks` property will accept a [query](/concepts/queries) to allow you to filter, and sort results.
...
```ts todo-item/todo-item.module.ts
import { NestjsQueryGraphQLModule } from '@ptc-org/nestjs-query-graphql'
import { NestjsQueryTypeOrmModule } from '@ptc-org/nestjs-query-typeorm'
import { Module } from '@nestjs/common'
...
<Note>
  If `remove.enabled` was set to `true` a `removeSubTasksFromTodoItem` mutation would also be exposed with the same arguments as
  `addSubTasksToTodoItem`.
</Note>

## @FilterableUnPagedRelation[​](#filterableunpagedrelation 'Direct link to @FilterableUnPagedRelation')
...
## @OffsetConnection[​](#offsetconnection 'Direct link to @OffsetConnection')
...
- `todoItem.subTasks` - A `query` to retrieve a `TodoItems` `SubTasks`.
  ...
  - The `subTasks` property will return a offset based [connection](/graphql/types#connectiontype) to page through results.
...
### Total Count Example[​](#total-count-example 'Direct link to Total Count Example')
...
## @FilterableOffsetConnection[​](#filterableoffsetconnection 'Direct link to @FilterableOffsetConnection')
...
<Warning>
  ...
  `sequelize`). If your relations are federated or you are using `mongoose` you cannot use the `@FilterableConnection` decorator.
</Warning>
...
## @CursorConnection[​](#cursorconnection 'Direct link to @CursorConnection')
...
- `todoItem.subTasks` - A `query` to retrieve a `TodoItems` `SubTasks`.
  ...
  - The `subTasks` property will accept a [query](/concepts/queries) to allow you to filter, page and sort results.
  - The `subTasks` property will return a cursor based [connection](/graphql/types#connectiontype) to page through results.
...
### Total Count Example[​](#total-count-example-1 'Direct link to Total Count Example')
...
## @FilterableCursorConnection[​](#filterablecursorconnection 'Direct link to @FilterableCursorConnection')
...
## Virtual Relations[​](#virtual-relations 'Direct link to Virtual Relations')

You may run into a case where you have a `virtual` relation that does not exist in the database. `nestjs-query` supports this through the [RelationQueryService](/concepts/services#relationqueryservice).
...
<Note>
  ...
  [https://docs.nestjs.com/graphql/tooling#execute-enhancers-at-the-field-resolver-level](https://docs.nestjs.com/graphql/tooling#execute-enhancers-at-the-field-resolver-level)
</Note>

### Custom Relation Name[​](#custom-relation-name 'Direct link to Custom Relation Name')
...
### GraphQL lookahead[​](#graphql-lookahead 'Direct link to GraphQL lookahead')
...
### Disable Reads[​](#disable-reads 'Direct link to Disable Reads')
...
#### Disable `filter` or `sorting` in relations[​](#disable-filter-or-sorting-in-relations 'Direct link to disable-filter-or-sorting-in-relations')
...
### Enable Updates[​](#enable-updates 'Direct link to Enable Updates')
...
### Enable Removes[​](#enable-removes 'Direct link to Enable Removes')
...
### Guards, Pipes and Filters[​](#guards-pipes-and-filters 'Direct link to Guards, Pipes and Filters')

**NOTE** `guards`, `pipes`, `interceptors` and `filters` will not work by default with `read` endpoints. See [https://github.com/nestjs/graphql/issues/295](https://github.com/nestjs/graphql/issues/295)
...
### Relation Mixin[​](#relation-mixin 'Direct link to Relation Mixin')

If you are using the [resolvers individually](/graphql/resolvers#individual-resolvers) you can use the following mixins to add relations functionality.

#### Relatable[​](#relatable 'Direct link to Relatable')
...
```ts todo-item/todo-item.resolver.ts

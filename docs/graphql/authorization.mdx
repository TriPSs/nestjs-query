...
```ts todo-item/dto/todo-item.dto.ts
...
```ts sub-task/dto/sub-task.dto.ts
...
```ts sub-task/sub-task.authorizer.ts
...
- `queryMany`
- `findById`
- `exportMany` (only when `export.enabled` is `true`)
- `aggregate`
- `createOne`
- `createMany`
- `updateOne`
...
To enforce the auth filter on create mutations, set the `validateWithAuthFilter` option in the resolver's `create` options. The filter is then passed to the `QueryService`, which evaluates it in JS against each input before saving. The option only takes effect when the authorizer resolves a filter for the `CREATE` operation group; if no authorizer is registered for the DTO, or it returns no filter for create operations, the option is a no-op.
...
With the `TodoItemDTO` from the [`@Authorize` decorator](#authorize-decorator) example, where `authorize` returns `{ ownerId: { eq: context.req.user.id } }`, a `createOneTodoItem` mutation with an `ownerId` that does not match the authenticated user is now rejected.

The two mutations behave differently when an input does not match the filter:

- `createOne` throws a `BadRequestException` (GraphQL error code `BAD_REQUEST`) with the message `Entity does not meet creation constraints`, and nothing is created.
- `createMany` silently skips inputs that do not match the filter and only creates (and returns) the matching subset.

You can also set the option per mutation through the `one` and `many` options. A per-mutation flag takes precedence over the top-level flag (the same nullish-coalescing behavior as `enableSubscriptions`), so you can enable the validation for both mutations and opt one of them back out:
...
This enforces the filter on `createOne` but not on `createMany`.

<Warning>
  The auth filter is evaluated in JS against the plain input object, not by the database. This has a few limitations:
  ...
  - The filter is evaluated against the fields present on the input: a field that is missing from the input fails positive

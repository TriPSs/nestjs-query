...
<Note>Only fields decorated with `@FilterableField` will be exposed in aggregate queries.</Note>
...
<Info>`nestjs-query` will only expose number fields for `avg` and `sum`.</Info>

```graphql
type Query {
  todoItem(id: ID!): TodoItem
  todoItems(paging: CursorPaging = { first: 10 }, filter: TodoItemFilter = {}, sorting: [TodoItemSort!] = []): TodoItemConnection!

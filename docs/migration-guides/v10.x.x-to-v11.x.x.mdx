...
In v11, a filter that cannot be read as either a comparison or a nested filter raises an `InvalidFilterError` rather than being evaluated to a result. Two shapes are affected:

...
## QueryService.nullOrdering

If you implement `QueryService` directly rather than extending a provided service, you can add a `nullOrdering` member so keyset pagination walks nullable sort fields in full. Leave it `undefined` to keep the previous behaviour.

Keyset pagination uses it to place boundaries over nullable sort fields, so cursors minted by v10 over such a sort return different rows at the boundary.

Every keyset sort field must be loaded on the returned nodes. A field that is not loaded, such as a TypeORM `select: false` column, can now make paging repeat rows or never end. See [Nullable sort fields](/graphql/queries/paging#nullable-sort-fields).

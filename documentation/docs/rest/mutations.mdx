...
You can also supply custom `CreateOneInput` or `UpdateOneInput` classes when the generated body type is not sufficient. For application-level transformations, use [hooks](./hooks.mdx); for record-level access rules, use [authorization](./authorization.mdx).

...
Use `AssemblerClass` instead of `EntityClass` when an [assembler](../concepts/advanced/assemblers.mdx) maps the entity and DTO, and register the assembler in the module's `assemblers` array. Use `ServiceClass` when the endpoint is backed directly by a custom `QueryService`, and register that provider in `services`.
...
```ts title="todo-item.controller.ts"

...
Use `AssemblerClass` instead of `EntityClass` when an [assembler](../concepts/advanced/assemblers.mdx) maps the entity and DTO. Use `ServiceClass` when the endpoint is backed directly by a custom `QueryService`.

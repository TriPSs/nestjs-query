...
`disableFilter` removes generated filter query parameters. Authorization filters are still applied by the server.
...
See [hooks](../hooks.mdx) for a complete query hook example.

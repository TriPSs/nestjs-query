...
import { Query, mergeFilter } from '@ptc-org/nestjs-query-core'
import { BeforeQueryMany, FilterableField } from '@ptc-org/nestjs-query-rest'

@BeforeQueryMany((query: Query<TodoItemDTO>) => {
  if (!query.query) {
    return query
  }

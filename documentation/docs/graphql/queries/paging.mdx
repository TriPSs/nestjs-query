...
### Using Offset with Cursors

You can combine `offset` with cursor pagination to skip additional records. This is useful when you need to skip a specific number of records after a cursor position.
...
In this example, pagination starts after the cursor position, then skips 3 more records before returning the next 2 items.

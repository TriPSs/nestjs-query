...
Notice the use of `@FilterableField` this will let `@souagrosolucoes/nestjs-query-graphql` know to allow filtering on the
...
From the previously created DTO, `@souagrosolucoes/nestjs-query-graphql` will automatically create a `CreateTodoItem` graphql type:

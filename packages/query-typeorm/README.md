<p align="center">
  <a href="https://nestjs-query.mintlify.site/" target="blank"><img src="https://nestjs-query.mintlify.site/images/logo.svg" width="120" alt="Nestjs-query Logo" /></a>
</p>

[![npm version](https://img.shields.io/npm/v/@ptc-org/nestjs-query-typeorm.svg)](https://www.npmjs.org/package/@ptc-org/nestjs-query-typeorm)
[![Test](https://github.com/tripss/nestjs-query/workflows/Test/badge.svg?branch=master)](https://github.com/tripss/nestjs-query/actions?query=workflow%3ATest+and+branch%3Amaster+)
[![Coverage Status](https://codecov.io/gh/TriPSs/nestjs-query/branch/master/graph/badge.svg?token=29EX71ID2P)](https://codecov.io/gh/TriPSs/nestjs-query)
[![Known Vulnerabilities](https://snyk.io/test/github/tripss/nestjs-query/badge.svg?targetFile=packages/query-typeorm/package.json)](https://snyk.io/test/github/tripss/nestjs-query?targetFile=packages/query-typeorm/package.json)

# `@ptc-org/nestjs-query-typeorm`

The `query-typeorm` package provides an implementation of the `@ptc-org/nestjs-query-core` `QueryService`, built on [NestJS](https://nestjs.com/) and [TypeORM](https://typeorm.io/).

## Installation

[Install Guide](https://nestjs-query.mintlify.site/introduction/install)

## Getting Started

To get started with `@ptc-org/nestjs-query-typeorm`, check out the [Getting Started](https://nestjs-query.mintlify.site/persistence/typeorm/getting-started) guide.

...
import { ObjectLiteral, RelationQueryBuilder as TypeOrmRelationQueryBuilder, Repository } from 'typeorm';
...
export abstract class RelationQueryService<Entity> {
	...
	public async queryRelations<Relation extends object>(
		...
		return relationQueryBuilder.select(dto, query).getMany();
	}
	...
	public async aggregateRelations<Relation extends object>(
		...
		return AggregateBuilder.asyncConvertToAggregateResponse(
			relationQueryBuilder
				.aggregate(dto, { filter }, aggregate)
	...
	public async countRelations<Relation extends object>(
		...
		const relationQueryBuilder = this.getRelationQueryBuilder<Relation>(relationName);
		return relationQueryBuilder.select(dto, { filter }).getCount();
	}
	...
	public async findRelation<Relation extends object>(
		...
		const relationQueryBuilder = this.getRelationQueryBuilder<Relation>(relationName).select(dto, {
			filter: opts?.filter,
			paging: { limit: 1 },
		});
		...
		return relationQueryBuilder.getOne();
	}
	...
	private async batchQueryRelations<Relation extends object>(
		...
		const relationQueryBuilder = this.getRelationQueryBuilder<Relation>(relationName);
		const entityRelations = await relationQueryBuilder.batchSelect(entities, query, withDeleted).getRawAndEntities();
		...
		return entityRelations.raw.reduce((results: Map<Entity, Relation[]>, rawRelation: EntityIndexRelation<unknown>) => {
			...
			const relationDtos = this.getRelationsFromPrimaryKeys(relationQueryBuilder, rawRelation, entityRelations.entities)
				.map(r => Object.assign(new RelationClass(), r));
	...
	private async batchAggregateRelations<Relation extends object>(
		...
		const rawAggregates = await relationQueryBuilder
			.batchAggregate(entities, { filter }, query)
	...
	private async batchCountRelations<Relation extends object>(
		...
		const entityRelations = await Promise.all(entities.map((e) => relationQueryBuilder.select(e, { filter }).getCount()));
	...
	private getRelationMeta(relationName: string) {
	...
	private getRelationEntity(relationName: string): Class<object> {
		...
		if (typeof relationMeta.type === 'string') {
			const relationMetaData = this.repo.manager.connection.entityMetadatas.find((em) => em.targetName === String(relationMeta.type));

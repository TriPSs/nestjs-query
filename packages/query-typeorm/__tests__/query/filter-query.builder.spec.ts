    ...
    describe('with relation', () => {
      ...
      it('should bind the filter to the alias the filtered relation was joined under', () => {
        const selectQueryBuilder = new FilterQueryBuilder(connection.getRepository(TestEntity)).select({
          filter: { testRelations: { testEntity: { manyTestRelations: { relationName: { eq: 'foo' } } } } },
          relations: [{ name: 'manyTestRelations', query: {} }]
        })
        const [sql, params] = selectQueryBuilder.getQueryAndParameters()

        expect(sql).toContain('LEFT JOIN "test_relation" "manyTestRelations" ON')
        expect(sql).toContain('LEFT JOIN "test_relation" "manyTestRelations_1" ON')
        expect(sql).toContain('"manyTestRelations_1"."relation_name" =')
        expect(sql).not.toContain('"manyTestRelations"."relation_name" =')
        expect(formatSql(sql, { params })).toMatchSnapshot()
      })

      it('should share a single alias and join when a filter and a selected relation name the same relation', () => {
        const selectQueryBuilder = new FilterQueryBuilder(connection.getRepository(TestEntity)).select({
        ...
        expect(sql).toContain('"manyTestRelations"."relation_name" =')
        expect(sql).not.toContain('manyTestRelations_1')
      ...
      it('should apply the filter of a selected relation through applyFilter', () => {
        ...
        class RecordingFilterQueryBuilder<Entity> extends FilterQueryBuilder<Entity> {
          public applyFilter<Where extends WhereExpressionBuilder>(qb: Where, filter?: Filter<Entity>, alias?: string): Where {
            aliasesFilteredFor.push(alias)

            return super.applyFilter(qb, filter, alias)
          }
        }

        const [sql] = new RecordingFilterQueryBuilder(connection.getRepository(TestEntity))
          .select({
            relations: [selectRelation<TestRelation, TestEntity>('testRelations', { filter: { relationName: { eq: 'foo' } } })]
          })
          .getQueryAndParameters()

        expect(sql).toContain('"testRelations"."relation_name" = ?')
        expect(aliasesFilteredFor).toEqual(['testRelations', 'TestEntity'])
      })

      it('should apply the filter of a selected relation given without metadata', () => {
        const repo = connection.getRepository(TestEntity)
        const builder = new FilterQueryBuilder(repo)

        const [sql] = builder
          .applyRelationJoinsRecursive(repo.createQueryBuilder(), { testRelations: { alias: 'testRelations' } }, [
            selectRelation<TestRelation, TestEntity>('testRelations', { filter: { relationName: { eq: 'foo' } } })
          ])
          .getQueryAndParameters()
      ...
      it('should call applyFilter for a selected relation without a filter', () => {
        const aliasesFilteredFor: (string | undefined)[] = []
        ...
        new RecordingFilterQueryBuilder(connection.getRepository(TestEntity)).select({
          relations: [selectRelation<TestRelation, TestEntity>('testRelations', {})]
        })

...
import {
	CursorConnection, CursorQueryArgsType,
	FederationResolver,
	FilterableField,
	OffsetConnection, OffsetQueryArgsType,
	Relation,
	UnPagedRelation,
...
describe('FederationResolver', () => {
	...
	interface FinalizedTestResolver {
		findCustom(dto: TestResolverDTO, filter: object): Promise<TestRelationDTO>;

		queryRelationCursorConnections(dto: TestResolverDTO, query: CursorQueryArgsType<TestRelationDTO>, filter: object): any;

		queryRelationOffsetConnections(dto: TestResolverDTO, query: CursorQueryArgsType<TestRelationDTO>, filter: object): any;

		queryUnPagedRelations(dto: TestResolverDTO, query: CursorQueryArgsType<TestRelationDTO>, filter: object): any;
	}
		...
		describe('one relation', () => {
			it('should call the service findRelation with the provided dto and correct relation name', async () => {
				const {
					resolver,
					mockService,
				} = await createResolverFromNest<TestResolver, FinalizedTestResolver>(TestResolver, TestFederatedDTO);

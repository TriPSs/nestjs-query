...
describe('ReadResolver', () => {
	const expectResolverSDL = async (opts?: ReadResolverOpts<TestResolverDTO>) => {
		...
		class TestSDLResolver extends ReadResolverFactory(TestResolverDTO, opts) {
	...
	describe('#queryMany', () => {
		...
		describe('#queryMany cursor connection', () => {
			@Resolver(() => TestResolverDTO)
			class TestResolver extends ReadResolverFactory(TestResolverDTO) {
			...
			it('should call the service query with the provided input', async () => {
				...
				when(mockService.query(objectContaining({
					...input,
			...
			it('should merge the filter an auth filter if provided', async () => {
				...
				when(
					mockService.query(
						objectContaining({
							filter: { and: [input.filter, authorizeFilter] },
							paging: { limit: 2, offset: 0 },
						}),
					),
				).thenResolve(output);
		...
		describe('#queryMany array connection', () => {
			@Resolver(() => TestResolverDTO)
			class TestResolver extends ReadResolverFactory(TestResolverDTO, { pagingStrategy: PagingStrategies.OFFSET }) {
				constructor(@Inject() service: TestService) {
					super(service);
				}
			}
		...
		describe('#queryMany no paging connection', () => {
			@Resolver(() => TestResolverDTO)
			class TestResolver extends ReadResolverFactory(TestResolverDTO, { pagingStrategy: PagingStrategies.NONE }) {
				constructor(@Inject() service: TestService) {
					super(service);
				}
			}
	...
	describe('#findById', () => {
		...
		it('should call the service getById with soft delete on when enabled', async () => {
			@Resolver(() => TestResolverDTO)
			class TestResolverTwo extends ReadResolverFactory(TestResolverDTO, {
	...
	it('should expose totalCount on cursor connections if enableTotalCount is true', async () => {
		@Resolver(() => TestResolverDTO)
		class TestTotalCountSDLResolver extends ReadResolverFactory(TestResolverDTO, { enableTotalCount: true }) {
			@Query(() => TestResolverDTO)
			test(): TestResolverDTO {
				return { id: '1', stringField: 'foo' };
			}
	...
	it('should expose totalCount on offset connections if enableTotalCount is true', async () => {
		@Resolver(() => TestResolverDTO)
		class TestTotalCountSDLResolver extends ReadResolverFactory(TestResolverDTO, {
			pagingStrategy: PagingStrategies.OFFSET,

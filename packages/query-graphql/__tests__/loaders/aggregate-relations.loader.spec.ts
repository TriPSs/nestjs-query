import { AggregateQuery, QueryService } from '@rezonate/nestjs-query-core';
...
describe('AggregateRelationsLoader', () => {
	describe('createLoader', () => {
		...
		it('should try to load the relations with the query args', () => {
			...
			when(
				service.aggregateRelations(RelationDTO, 'relation', deepEqual(dtos), deepEqual(filter), deepEqual(aggregate), undefined, undefined, undefined, undefined),
			).thenResolve(
				new Map([
		...
		it('should group queryRelations calls by filter and return in the correct order', () => {
			...
			when(
				service.aggregateRelations(
					...
					deepEqual(filter1),
					deepEqual(aggregate), undefined, undefined, undefined, undefined
				),
			).thenResolve(
				new Map([
					[dtos[0], dto1Aggregate],
		...
		it('should group queryRelations calls by aggregate and return in the correct order', () => {
			...
			when(
				service.aggregateRelations(
					...
					deepEqual([dtos[0], dtos[2]]),
					deepEqual(filter),
					deepEqual(aggregate1), undefined, undefined, undefined, undefined
				),
			).thenResolve(
				new Map([
					[dtos[0], dto1Aggregate],
			...
			when(
				service.aggregateRelations(
					...
					deepEqual([dtos[1], dtos[3]]),
					deepEqual(filter),
					deepEqual(aggregate2), undefined, undefined, undefined, undefined,
				),
			).thenResolve(
				new Map([
					[dtos[1], dto2Aggregate],

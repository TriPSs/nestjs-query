...
export const truncate = async (db: DataSource): Promise<void> => executeTruncate(db, tables)

export const refresh = async (db: DataSource): Promise<void> => {
  await truncate(db)

  const todoRepo = db.getRepository(TodoItemEntity)
  const subTaskRepo = db.getRepository(SubTaskEntity)
  const tagsRepo = db.getRepository(TagEntity)

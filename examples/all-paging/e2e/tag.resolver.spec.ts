...
import { cursorPageInfoField, edgeNodes, offsetConnection, tagFields, todoItemFields } from './graphql-fragments'
    ...
    describe('PagingStrategies.OFFSET', () => {
      it(`should return a connection`, () =>
        request(app.getHttpServer())
          ...
          .expect(200, { data: { tagsOffset: { nodes: tags, pageInfo: { hasNextPage: false, hasPreviousPage: false } } } }))
      ...
      it(`should allow querying`, () =>
        request(app.getHttpServer())
          ...
          .send({
            ...
            variables: {},
            query: `{
          tagsOffset(filter: { id: { in: [1, 2, 3] } }) {
            ${offsetConnection(tagFields)}
          }
        }`
          })
          .expect(200, {
            data: { tagsOffset: { nodes: tags.slice(0, 3), pageInfo: { hasNextPage: false, hasPreviousPage: false } } }
          }))
      ...
      it(`should allow sorting`, () =>
        request(app.getHttpServer())
          ...
          .send({
            ...
            variables: {},
            query: `{
          tagsOffset(sorting: [{field: id, direction: DESC}]) {
            ${offsetConnection(tagFields)}
          }
        }`
          })
          .expect(200, {
            data: { tagsOffset: { nodes: tags.slice().reverse(), pageInfo: { hasNextPage: false, hasPreviousPage: false } } }
          }))
        ...
        it(`should allow paging with the 'limit' field`, () =>
          request(app.getHttpServer())
            ...
            .send({
              ...
              variables: {},
              query: `{
          tagsOffset(paging: {limit: 2}) {
            ${offsetConnection(tagFields)}
          }
        }`
            })
            .expect(200, {
              data: { tagsOffset: { nodes: tags.slice(0, 2), pageInfo: { hasNextPage: true, hasPreviousPage: false } } }
            }))
        ...
        it(`should allow paging with the 'limit' field and 'offset'`, () =>
          request(app.getHttpServer())
            ...
            .send({
              ...
              variables: {},
              query: `{
          tagsOffset(paging: {limit: 2, offset: 2}) {
            ${offsetConnection(tagFields)}
          }
        }`
            })
            .expect(200)
            .expect(200, {
              data: { tagsOffset: { nodes: tags.slice(2, 4), pageInfo: { hasNextPage: true, hasPreviousPage: true } } }
            }))
      })
    })
    describe('PagingStrategies.CURSOR', () => {
      it(`should return a connection`, () =>
        request(app.getHttpServer())
          ...
          .expect(200)
          .then(({ body }) => {
            const { edges, pageInfo }: CursorConnectionType<TagDTO> = body.data.tagsCursor
      ...
      it(`should allow sorting`, () =>
        request(app.getHttpServer())
          ...
          .send({
            ...
            variables: {},
            query: `{
          tagsCursor(sorting: [{field: id, direction: DESC}]) {
            ${cursorPageInfoField}
            ${edgeNodes(tagFields)}
          }
        }`
        ...
        it(`should allow paging with the 'first' field and 'after'`, () =>
          request(app.getHttpServer())
            ...
            .send({
              ...
              variables: {},
              query: `{
          tagsCursor(paging: {first: 2, after: "YXJyYXljb25uZWN0aW9uOjE="}) {
            ${cursorPageInfoField}
            ${edgeNodes(tagFields)}
          }
        }`
